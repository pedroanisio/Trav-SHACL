...
The supported SHACL Core constraint families now include cardinality, datatype, value-type (`sh:class`, `sh:nodeKind`), value-range (`sh:minInclusive`, `sh:minExclusive`, `sh:maxInclusive`, `sh:maxExclusive`), string (`sh:minLength`, `sh:maxLength`, `sh:pattern`, `sh:languageIn`, `sh:uniqueLang`), property-pair (`sh:equals`, `sh:disjoint`, `sh:lessThan`, `sh:lessThanOrEquals`), qualified value shape constraints, closed shapes, direct value constraints (`sh:hasValue`, `sh:in`), simple `sh:or`, and simple `sh:sparql` constraints. Logical operators are supported with shape-reference inners: `sh:and` (P2C/P3B), `sh:not` atomic property-shape inner (P2C-A), and `sh:xone` with the K-02 known limitation (P3B engine fix; strict exactly-one semantics deferred to Phase 4+).

A spec-conformant `sh:ValidationReport` serializer (Turtle or JSON-LD) ships in P3A: pass `report_format='turtle'` or `'jsonld'` to `ShapeSchema.validate` to receive `(dict, report_str)` instead of the default per-shape dict. The JSON shape format is **formally deprecated** and will be removed in v2.0.0 — see [docs/adr/ADR-010-json-format-deprecation.md](./docs/adr/ADR-010-json-format-deprecation.md). See [cover-report-phase-3.md](./cover-report-phase-3.md) for the full compliance status and residual limitations.
