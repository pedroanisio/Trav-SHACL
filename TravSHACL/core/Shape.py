...
class Shape:
    ...
    def __compute_predicate_set(self, min_query_id, max_queries_ids):
        ...
        return [self.id] + [self.id + "_d1"] + [min_query_id] + list(max_queries_ids)
    ...
    def __compute_target_queries(self):
        ...
        self.targetQuery = self.QueryGenerator.generate_target_query(
            "plain_target", None, self.targetQueryNoPref, self.includePrefixes, self.ORDERBYinQueries
        )

        self.queriesFilters = {}
        for ref in self.referencedShapes:
            for c in self.constraints:
                if c.path_sparql() == self.referencedShapes[ref]:
                    query_valid, query_invalid = self.QueryGenerator.generate_target_query(
                        "template_FILTER", [c], self.targetQueryNoPref, self.includePrefixes, self.ORDERBYinQueries
                    )
                    ref_dict = {"query_valid": query_valid, "query_invalid": query_invalid, "constraint": c}
    ...
    def compute_constraint_queries(self):
        ...
        i = itertools.count()
        self.maxQueries = [
            self.QueryGenerator.generate_query(
                max_ids[next(i)],
                [c],
                self.useSelectiveQueries,
                self.targetQueryNoPref,
    ...
    def get_or_query(self):
        if True in self.flag:
            ...
            if len(or_constraints) == 0:
                ...
                return self.QueryGenerator.options_query(
                    or_constraints, self.targetQuery, self.includePrefixes, self.ORDERBYinQueries
                )
        return
