...
class Validation:
    ...
    def validate(self, state, focus_shape):
        ...
        if focus_shape.is_deactivated():
            ...
            state.remaining_targets = {
                target for target in state.remaining_targets if target[0] != focus_shape.get_id()
            }
    ...
    def retrieve_next_targets(self, state, next_focus_shape, shapes_state):
        ...
        max_ref = (
            next_focus_shape.is_max_ref(filtering_shape.get_id()) if filtering_shape is not None else True
        )  # TODO: check if filtering based on max constraints is possible
        ...
        if next_focus_shape.flag:
            ...
            for target in pending:
                if pending_val and target not in pending_val:
        ...
        if len(sparql_constraints) > 0:
            for constraint in sparql_constraints:
                violations = self.InstRetrieval.execute_sparql_constraint(
                    constraint_id=constraint.id, query_str=constraint.query, instance_list=[p[1] for p in pending]
                )
    ...
    def get_evaluated_out_neighbor(focus_shape, visited_shapes, state):
        ...
        for prev_shape in visited_shapes:
            ...
            if focus_shape.referencedShapes.get(prev_shape_name) is not None:
                ...
                if (
                    ((0 < len_val < best_val_threshold) or (0 < len_inv < best_inv_threshold))
                    and len_inv > 0
                    and prev_shape.get_target_query() is not None
                    and state.shapes_state[prev_shape_name]["remaining_targets_count"] == 0
                ):
    ...
    def interleave(self, state, shape, q, filtering_shape, q_rule_pattern, s_rule_pattern, q_type):
        ...
        for query_str in self.InstRetrieval.rewrite_constraint_query(
            shape, q, filtering_shape, q_type, self.selectivity_enabled
        ):
            start = time.time() * 1000.0
            for b in self.InstRetrieval.run_constraint_query(q, query_str):
                ...
                for i, atom_pattern in enumerate(query_rp_body):
                    ...
                    if a[0] in state.evaluated_predicates:  # exclude not yet evaluated atom's query
                        ...
                        if (
                            a in a_state["inferred"]
                            and q_type == "min"
                            and shape_max_refs.get(q_body_ref_shapes[i]) is not None
                        ):
                            if inter_constr_count.get(s_head) is None:
                                inter_constr_count[s_head] = {
                                    inter_constraint: set() for inter_constraint in shape_max_refs
                                }
                            inter_constr_count[s_head][q_body_ref_shapes[i]].add(a)
                            if (
                                len(inter_constr_count[s_head][q_body_ref_shapes[i]])
                                > shape_max_refs[q_body_ref_shapes[i]]
                            ):
                ...
                if not is_body_inferred:
                    if is_body_inferrable:
                        if q_head not in state.rule_map:
                            s = set()
                            s.add(frozenset(body))
    ...
    def saturate_remaining(self, state, shape_name, shapes_state):
        ...
        negated = self.negate_unmatchable_heads(
            state, rule_map, state.evaluated_predicates, shape_name, shapes_state, state.preds_to_shapes
        )
        inferred = self.apply_rules(
            state, state.remaining_targets, rule_map, shape_name, shapes_state, state.preds_to_shapes
        )
        if negated or inferred:
            self.saturate_remaining(state, shape_name, shapes_state)

    def negate_unmatchable_heads(
        self, state, rule_map, evaluated_predicates, shape_name, shapes_state, preds_to_shapes
    ):
        """
        ...
        for a in all_body_atoms:
            ...
            if (
                a[0] in evaluated_predicates
                and rule_map.get((a[0], a[1], True)) is None
                and a not in a_state["inferred"]
            ):  # if atom 'a' is not satisfied
    ...
    def apply_rules(self, state, remaining_targets, rule_map, shape_name, shapes_state, preds_to_shapes):
        ...
        for head, bodies in rule_map_copy.items():
            ...
            for body in bodies:
                inferred_atoms = {
                    "F_atom"
                    if (a[0], a[1], not a[2]) in shapes_state[preds_to_shapes[a[0]]]["inferred"]
                    else ("T_atom" if a in shapes_state[preds_to_shapes[a[0]]]["inferred"] else "P_atom")
                    for a in body
                }
