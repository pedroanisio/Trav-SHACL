...
class SPARQLEndpoint:
    ...
    class __SPARQLEndpoint:
        ...
        def __init__(self, url: str | Graph, user: str | None = None, pwd: str | None = None):
            if isinstance(url, str):
                self.endpoint: SPARQLWrapper | Graph = SPARQLWrapper(url)
        ...
        def run_query(self, query_string):
            if isinstance(self.endpoint, SPARQLWrapper):
                ...
                result_dict = {"head": {"vars": [v.toPython()[1:] for v in variables]}, "results": {"bindings": []}}
    ...
    def __new__(cls, endpoint, user, pwd):
        if not isinstance(endpoint, (str, Graph)):
            raise TypeError(
                "The SPARQL endpoint needs to be a URL (as string) or an in-memory RDFlib graph. "
                + type(endpoint)
                + " given instead."
            )
